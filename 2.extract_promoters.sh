...
then
	...
	wget https://ftp.ensembl.org/pub/release-84/fasta/homo_sapiens/dna/Homo_sapiens.GRCh38.dna.primary_assembly.fa.gz -P ./annotation/ ;
	gzip -d ./annotation/Homo_sapiens.GRCh38.dna.primary_assembly.fa.gz ;
	mv ./annotation/Homo_sapiens.GRCh38.dna.primary_assembly.fa ./annotation/hg38.fa;
	#sed -i 's/^>/>chr/g' ./annotation/hg38.fa;
fi
...
then
	...
	ref="./annotation/hg38.fa"; # Genome used for alignment purpose
	anot="./annotation/Homo_sapiens.GRCh38.84.chr_complete_annotation.txt"; # Gene annotations used for quantification purpose
	...
	COLS=(`grep -Ff $sub_group $anot | grep -v "^#" | awk '!x[$0]++' | sed 's/\t/;/g'`); # Selecting complete gene annotations for "lncRNA_Down" genes (by ENSGxxxx ids)
	...
	do
		...
		#samtools faidx --continue $ref chr$chromosome:$pstart-$pend | sed "s/>chr$chromosome:$pstart-$pend/>${oname} chr$chromosome:$pstart-$pend/"; #; promoter chr$chromosome:${pstart}-${pend}/"; # if chromosome numbers are chr1,chr2,chr3 etc.,
	done
	## Bash script (END)
